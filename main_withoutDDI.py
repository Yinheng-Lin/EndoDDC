...
def find_free_port():
    ...
    sock.bind(('', 0))  # 绑定到一个随机可用端口
    port = sock.getsockname()[1]
    sock.close()
    return port

def train(gpu, args):
    # 在初始化分布式进程组之前，找到一个可用端口
    if not args.multiprocessing:
        if args.tcp_port <= 0:  # 如果端口设置为0或负数，自动寻找可用端口
            args.tcp_port = find_free_port()
            print(f"自动选择可用端口: {args.tcp_port}")
        
        # 使用重试机制初始化进程组
        max_attempts = 5
        for attempt in range(max_attempts):
            try:
                current_port = args.tcp_port + attempt
                print(f"尝试使用端口 {current_port} 进行分布式初始化")
                dist.init_process_group(
                    backend='nccl', 
                    init_method=f'tcp://localhost:{current_port}',
                    world_size=args.num_gpus, 
                    rank=gpu
                )
                print(f"成功使用端口 {current_port} 初始化分布式进程组")
                break
    ...
    for epoch in range(args.start_epoch, args.epochs+1):
        ...
        for batch, sample in enumerate(loader_train):
            ...
            # 检查损失值是否为NaN
            if torch.any(torch.isnan(loss_sum)) or torch.any(torch.isnan(loss_val)):
                print(f"NaN detected in loss at epoch {epoch}, batch {batch}")
                break
            ...
            for param in net.parameters():
                if param.grad is not None and torch.any(torch.isnan(param.grad)):
                    print(f"NaN detected in gradients at epoch {epoch}, batch {batch}")
                    break
            ...
            if gpu == 0:
                ...
                list_lr = [round(g['lr'], 6) for g in optimizer.param_groups]
...
def test(args):
    ...
    if args.pretrain is not None:
        ...
        if key_m:
            print('Missing keys :')
            print(key_m)
            print("警告: 模型加载了部分参数，某些层将使用随机初始化值")
            # 注释掉下面这行，不再抛出异常
            # raise KeyError
