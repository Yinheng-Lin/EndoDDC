...
class OGNIDC(nn.Module):
    ...
    def forward(self, sample):
        ...
        for itr in range(self.GRU_iters):
            ...
            #####################################################################################################################
            output_path = "/hy-tmp/image_process"
            if not os.path.exists(output_path):
                os.makedirs(output_path)

            np.save(os.path.join(output_path, f"log_depth_pred.npy"), log_depth_pred.detach().cpu().numpy())
            #####################################################################################################################
            ...
            np.save(os.path.join(output_path, f"log_depth_up.npy"), log_depth_up.detach().cpu().numpy())
            ...
            np.save(os.path.join(output_path, f"depth_pred_up_init.npy"), depth_pred_up_init.detach().cpu().numpy())
            ...
            np.save(os.path.join(output_path, f"depth_pred_up_final.npy"), depth_pred_up_final.detach().cpu().numpy())
