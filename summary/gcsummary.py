...
class EndoDDCSummary(BaseSummary):
    ...
    def update(self, global_step, sample, output):
        ...
        eps = 1e-8
        log_gt = np.log(gt + eps)  # B x 1 x H x W
        log_gt[mask == 0.0] = 0.0
        ...
        log_gt_down = np.log(gt_down + eps)  # B x 1 x H x W
        ...
        for i in range(len(summary_img_name_list)):
            ...
            if img_tmp.dim() == 3:  # CxHxW
                img_np = img_tmp.numpy()
                img_np = np.transpose(img_np, (1, 2, 0))  # HxWxC
                img_np = (img_np * 255).astype(np.uint8)
                
                os.makedirs(os.path.join(self.log_dir, 'temp_images'), exist_ok=True)
                
                temp_path = os.path.join(self.log_dir, 'temp_images', f'temp_{i}.png')
                plt.imsave(temp_path, img_np)
                
                img = Image.open(temp_path)
                target_size = (800, int(800 * img.size[1] / img.size[0]))
                img = img.resize(target_size, LANCZOS)
                
                img_np = np.array(img).astype(np.float32) / 255.0
                if len(img_np.shape) == 2:
                    img_np = np.expand_dims(img_np, axis=2)
                img_tmp = torch.from_numpy(np.transpose(img_np, (2, 0, 1))).contiguous()

            try:
                self.add_image(self.mode + f'/{summary_img_name_list[i]}', 
                            img_tmp.float(), 
                            global_step,
                            dataformats='CHW')
        ...
        temp_dir = os.path.join(self.log_dir, 'temp_images')
