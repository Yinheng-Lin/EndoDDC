...
parser = argparse.ArgumentParser(description='EndoDDC')

# Dataset
parser.add_argument('--dir_data',
                    type=str,
                    default='/home/ren6/NAS/yinheng/dataset/StereoMIS/StereoMIS_test',
...
parser.add_argument('--split_json',
                    type=str,
                    default='/home/ren6/NAS/yinheng/dataset/StereoMIS/StereoMIS_test.json',
...
parser.add_argument('--beta_schedule',
                    type=str,
                    default='linear',
                    choices=('quad','linear','const','jsd','sigmoid'),
                    help='beta schedule type for diffusion')
parser.add_argument('--beta_start',
                    type=float,
                    default=0.0001,
                    help='start beta value for diffusion')
parser.add_argument('--beta_end',
                    type=float,
                    default=0.02,
                    help='end beta value for diffusion')
parser.add_argument('--num_diffusion_timesteps',
                    type=int,
                    default=1000,
                    help='number of diffusion timesteps')
parser.add_argument('--num_sampling_timesteps',
                    type=int,
                    default=20,
                    help='number of sampling timesteps')

# model related (for the diffusion UNet)
parser.add_argument('--model_in_channels',
                    type=int,
                    default=1,
                    help='input channels to the diffusion UNet')
parser.add_argument('--model_out_ch',
                    type=int,
                    default=1,
                    help='output channels of the diffusion UNet')
parser.add_argument('--model_ch',
                    type=int,
                    default=64,
                    help='base channel count for the diffusion UNet')
parser.add_argument('--model_ch_mult',
                    nargs="+",
                    type=int,
                    default=[1, 1, 2, 4],
                    # default=[1, 1, 1, 1],
                    help='channel multiplier for the diffusion UNet')
parser.add_argument('--model_num_res_blocks',
                    type=int,
                    default=2,
                    help='number of res blocks in each UNet stage')
parser.add_argument('--model_dropout',
                    type=float,
                    default=0.0,
                    help='dropout rate in the diffusion UNet')
parser.add_argument('--data_conditional',
                    type=bool,
                    default=True,
                    help='whether to condition input channels on additional feature')
parser.add_argument('--model_resamp_with_conv',
                    action='store_true',
                    default=True,
                    help='if true, use conv-based resample in diffusion UNet')
