...
# 该函数使用凸组合方法对深度图进行上采样。其核心思想是通过加权组合局部深度值来以因子r（默认值为8）放大深度图。通过滑动窗口和对窗口内的值应用softmax，得到加权的深度预测
...
class OGNIDC(nn.Module):
    # 初始化backbone，根据spn_type选择NLSPN和DySPN其中一种空间传播网络（SPN），用于通过邻域信息来精化深度预测
    ...
    # 初始化深度预测（log_depth_init）和梯度预测（log_depth_grad_init），初始值都设为零
    ...
    def forward(self, sample):
        # 前向传播从输入的sample中提取RGB图像、稀疏深度图（dep）和相机内参（K）
        rgb = sample['rgb']
        ...
        # 负责对深度图进行下采样，以减小更新阶段的计算负担。根据下采样方法（平均或最小值），会调整深度值
        ...
        # 根据指定的激活格式，深度图要么使用对数变换，要么直接使用原始深度图
        ...
        # 在训练时，为了增强模型对稀疏数据的鲁棒性，会随机遮蔽稀疏深度图中的一部分数据
        ...
        # backbone网络特征提取（使用backbone网络从RGB图像和遮蔽后的深度图中提取上下文信息。这个上下文将指导深度预测和精化过程）
        ...
        # initialization
        # log_depth_pred：代表预测得到的对数深度图。
        # log_depth_grad_pred_init：代表初始的对数深度梯度
        ...
        # 创建一个与 dep 形状相同的全零张量 b_init，并设置 requires_grad=True，表示该张量的梯度会被记录
        # 这个变量虽然在数值上没有实际意义，但用于在反向传播中引入额外的梯度记录或用于调试梯度流动
        b_init = torch.zeros_like(dep, requires_grad=True)

        # 创建了四个空列表，用于后续存储不同阶段或不同尺度的预测结果：
        #     log_depth_grad_predictions：存储对数深度梯度的预测结果（注释中说明初始值也会记录）。
        #     confidence_predictions：存储置信度的预测结果。
        #     depth_predictions_up：存储上采样后的深度预测结果。
        #     depth_predictions_up_initial：存储初始上采样深度预测结果
        log_depth_grad_predictions = [] # record the init value also
        confidence_predictions = []
        depth_predictions_up = []
        depth_predictions_up_initial = []

        # 使用DepthGradOptimLayer进行深度精化，通过前一次迭代的梯度优化深度预测
        for itr in range(self.GRU_iters):
            ...
            # BasicUpdateBlock 利用 ConvGRU 结构，根据当前的预测信息对深度梯度场进行迭代精化
            ...
            net, up_mask, delta_log_depth = self.update_depth(net_copy, inp_copy, log_depth_pred_whitened)
            log_depth_pred = log_depth_pred + delta_log_depth
